...
###############################################################################
# nginx WASM Dynamic build helper for lind-wasm-apps
#
# High-level strategy:
#   1. Configure nginx with WASM cross-compilation settings
#   2. Apply patches for WASI compatibility
#   3. Patch objs/Makefile to append -ldl to the link rule
#   4. Build nginx with wasm32-wasi toolchain as a Position Independent
#      Executable (PIE) — enables Lind's runtime dynamic loader to inject
#      shared libraries via --preload and support dlopen/dlsym/dlclose.
#   5. Optimize with wasm-opt (asyncify for multi-process support)
#   6. Precompile with lind-boot --precompile
#
# Prerequisites:
...
# -fPIC is required for Position Independent Code (PIE/dynamic binary)
...
# -Wl,--import-table               : import the indirect-function table (required for PIE)
# -Wl,--allow-undefined            : allow undefined symbols (resolved at load time)
# -Wl,--unresolved-symbols=import-dynamic : import remaining unresolved symbols dynamically
# -Wl,--export-dynamic             : export all symbols so the dynamic loader can find them
...
LDFLAGS_WASM="-fPIC -Wl,-pie \
    -Wl,--import-table \
    -Wl,--shared-memory,--import-memory,--export-memory,--max-memory=67108864 \
...
echo "[nginx] building as PIE (dynamic loading enabled)"
echo
...
###############################################################################
# 2.5. Patch objs/Makefile to append -ldl to the link rule
###############################################################################

# After configure, nginx generates objs/Makefile with a link rule that ends
# with $(LIBS). We append -ldl so the dynamic loader support library is linked
# in after all objects and archives, which is required for dlopen/dlsym/dlclose.
if [[ -f objs/Makefile ]]; then
    sed -i 's/\$(LIBS)$/$(LIBS) -ldl/' objs/Makefile
    echo "[nginx] patched objs/Makefile to add -ldl"
fi
...
echo "[nginx] building nginx with wasm32-wasi toolchain (PIE/dynamic)..."
...
# -ldl is appended to $(LIBS) via the objs/Makefile patch above.
make -j"$JOBS" \
...
echo
echo "[nginx] dynamic build complete. Outputs:"
